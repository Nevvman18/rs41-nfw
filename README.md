...
>
> 1. **Configure** - open the **Firmware Builder**, pick your board (`RSM4x4/4x5` or `RSM4x2/4x1`) and set your options with guided help. On the newer `RSM4x4/4x5` boards you can, for the best sensor accuracy, just type the measurement-boom serial and the original **Vaisala factory calibration** is pulled in for you; the older `RSM4x2/4x1` boards use the built-in NFW calibration (nothing to enter).
...
* `v69` - stability and polish on top of v68.
  ...
  * **Factory (Vaisala) calibration on `RSM4x2` / `RSM4x1`.** The older F100 boards now run the same **Vaisala factory calibration** as the new boards (the maths was moved to single precision so it fits the 64 KB flash), for manufacturer-grade temperature and humidity. They no longer use the NFW calibration model.
...
* `v68` - a big tooling, flashing and licensing release. Highlights:
  ...
  * **Sensor calibration by board.** Factory (Vaisala) calibration stays on the newer `RSM4x4` / `RSM4x5` (STM32L412), where you pick NFW or factory. The older `RSM4x2` / `RSM4x1` (STM32F100) run the proven **NFW calibration only**: the factory chain was trialled on the F100 but it does not fit and was found to hang the sonde in flight, so it is not built there and the builder hides the option for those boards (they need no calibration download).
