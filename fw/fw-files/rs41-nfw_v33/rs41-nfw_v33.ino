...
unsigned int pipWait = 0; //delay after pip transmission, SET TO 0 IF PIP NOT USED, in milliseconds, 0 - disable, 0<delay<5000 - standard delay, 5000<delay - delay + radio sleep mode and recalibration (if radioSleep enabled) and additional flight computing and gps receiving.
unsigned int powerSavePipWait = 0; //activates when the powerSave is ON, set to -1 to disable changing of the transmission delay above powerSaveAltitude
...
unsigned int horusWait = 0; //same as in pip but after horus
unsigned int powerSaveHorusWait = 0; //same as in pip but after horus
...
unsigned int horusSecondTransmissionWait = 0; //same
unsigned int powerSaveHorusSecondTransmissionWait = 0; //same
...
unsigned int aprsWait = 0;
unsigned int powerSaveAprsWait = 0;
...
unsigned int rttyWait = 0;
unsigned int powerSaveRttyWait = 0;
...
unsigned int morseWait = 0;
unsigned int powerSaveMorseWait = 0;
...
int powerSaveAltitude = 3000; //altitude in meters above which the powerSave features start to occur (currently, TX power is lowered from defaultRadioPwrSetting to powerSaveRadioPwrSetting and the transmision interval is changed from default delay to powerSave... delay), set to -1 to completely disable all powerSave features
...
//Sensor boom
bool sensorBoomEnable = false; //enables sensor boom measurement (currently only temperatures, wxHumidity is being engineered) and diagnostics
...
float vVCalc;
unsigned long modeChangeDelayCallbackTimer;
...
bool sensorBoomwxHumidityModuleError = false;
...
unsigned int pipWaitActual = pipWait;
unsigned int horusWaitActual = horusWait;
unsigned int horusSecondTransmissionWaitActual = horusSecondTransmissionWait;
unsigned int aprsWaitActual = aprsWait;
unsigned int morseWaitActual = morseWait;
unsigned int rttyWaitActual = rttyWait;
...
uint8_t ubxCfgNav5_maxPerformance[] = { //ublox 6-series max performance mode
  0xB5, 0x62, 0x06, 0x11, 0x02, 0x00, // Header/Command/Size
  0x08, 0x00,                         // Payload
  0x21, 0x91                          // Checksum
};

uint8_t ubxCfgNav5_powerSave[] = { // ublox 6-series powersave mode
  0xB5, 0x62, 0x06, 0x11, 0x02, 0x00, // Header/Command/Size
  0x08, 0x01,                         // Payload
  0x22, 0x92                          // Checksum
};
...
String createRttyMorsePayload() {
    ...
    if(sensorBoomWarn || sensorBoomErr) {
      rttyTemperature = static_cast<int>(readThermistorTemp());
    }
    else {
...
void deviceStatusHandler() {
  ...
  if(sensorBoomMainTempError && sensorBoomwxHumidityModuleError) {
    sensorBoomErr = false;
    sensorBoomWarn = true;
  }
  else if(sensorBoomMainTempError || sensorBoomwxHumidityModuleError) {
    sensorBoomWarn = true;
    sensorBoomErr = false;
  }
  else {
...
void disableRefHeaterRing() {
  ...
  digitalWrite(PULLUP_HYG, LOW); //disable wxHumidity ring oscillator power
...
void enableRefHeaterRing() {
  ...
  digitalWrite(PULLUP_HYG, LOW); //enable wxHumidity ring oscillator power
  ...
  if(powerSaveAltitude != -1) {
    if(gpsAlt > powerSaveAltitude) {
      ...
      if(powerSaveHorusWait != -1) {
        pipWaitActual = powerSaveHorusWait;
      }

      if(powerSaveHorusSecondTransmissionWait != -1) {
        pipWaitActual = powerSaveHorusSecondTransmissionWait;
      }

      if(powerSaveAprsWait != -1) {
        pipWaitActual = powerSaveAprsWait;
      }

      if(powerSaveMorseWait != -1) {
        pipWaitActual = powerSaveMorseWait;
      }

      if(powerSaveRttyWait != -1) {
        pipWaitActual = powerSaveRttyWait;
      }
...
void wxHumiditySensorHeaterControl(int heaterPower) { //0 - OFF, 1 - small power, 2 - full power
  if(heaterPower == 0) {
...
void sensorBoomHandler() {
  ...
  if(sensorBoomEnable) {
    ...
    mainwxTemperatureFrequency = getSensorBoomFreq(4);
    if(mainwxTemperatureFrequency <= 0) {
      ...
      mainTemperatureResistance = calculateSensorBoomResistance(mainwxTemperatureFrequency, tempSensorBoomCalibrationFactor);
    ...
    extHeaterwxTemperatureFrequency = getSensorBoomFreq(3);
    if(extHeaterwxTemperatureFrequency <= 0) {
      sensorBoomwxHumidityModuleError = true;  // Error if frequency is invalid
      if(xdataPortMode == 1) {
        xdataSerial.println("[WARN]: External wxHumidity heater temperature sensor error! MEAS frequency invalid.");
      }
    } else {
      sensorBoomwxHumidityModuleError = false;  // No error
      extHeaterTemperatureResistance = calculateSensorBoomResistance(extHeaterwxTemperatureFrequency, tempSensorBoomCalibrationFactor);
    ...
    sensorBoomGeneralError = sensorBoomMainTempError || sensorBoomwxHumidityModuleError;
...
void modeChangeDelayCallback(unsigned int waitTime) {
...
void aprsHabFormat(char* aprsMessage) {
    ...
    if(sensorBoomWarn || sensorBoomErr) {
      aprsTemperature = readThermistorTemp();
    }
    else {
...
void aprsWxFormat(float latitude, float longitude, char* aprsMessage) {
    ...
    int wxHumidity = 0;      // wxHumidity as percentage
    ...
    snprintf(aprsMessage, 200, 
         "!%s/%s_c%03ds%03dg%03dt%03dh%02db%05d [TempExt=%dC TempInt=%dC V=%dmV, %s]", 
         ...
         static_cast<int>(readThermistorTemp()), 
...
void aprsRecorderFormat(char* aprsMessage) {
    ...
    int aprsTemperature = static_cast<int>(
        (sensorBoomWarn || sensorBoomErr) ? readThermistorTemp() : mainTemperatureValue
    );
    ...
    int isHeaterOnInt = static_cast<int>(isHeaterOn ? 1 : 0);
    ...
    snprintf(
        ...
        "T%02d"
        "V%03d"
        " [NFW;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d; %s]",
        gpsAltFeet,
        aprsPacketNum,
        gpsSats,
        aprsTemperature,
...
void flightComputing() {
  ...
  if(static_cast<int>(gpsSpeed) > maxSpeed) {
    maxSpeed = static_cast<int>(gpsSpeed);
  }
  ...
  int tempInternal = static_cast<int>(readThermistorTemp());
...
void setup() {
  ...
  wxHumiditySensorHeaterControl(0); //turn off wxHumidity sensor heater
