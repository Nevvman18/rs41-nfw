...
bool referenceHeating = false; //This option enables slight warming up the reference heating resistors. This function works with the same method as the Vaisala firmware - maintaing temperature > 20*C of the cut out. When enabled, this should give just a slight improvement in temperature readings accuracy, increasing the power consumption a bit (suggested with 2xAA batteries)
...
void deviceStatusHandler() {
  ...
  if (vBatErr || sensorBoomErr) {
    err = true;
    ok = false;
  } else if (vBatWarn || gpsFixWarn || sensorBoomWarn || calibrationError) {
    warn = true;
    ok = false;
  } else {
...
void aprsLocationFormat(float latitude, float longitude, char* aprsMessage) {
  ...
  sprintf(latBuffer, "%02d%02d.%02d%c", latDegrees, latMinutes, latHMinutes, latHemisphere);
  ...
  sprintf(lonBuffer, "%03d%02d.%02d%c", lonDegrees, lonMinutes, lonHMinutes, lonHemisphere);
...
void aprsRecorderFormat(char* aprsMessage) {
    ...
    snprintf(
        ...
        "T%02d"
        "V%03d"
        " NFW;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d %s",
        gpsAltFeet,
        aprsPacketNum,
        gpsSats,
        aprsTemperature,
...
void flightComputing() {
  ...
  pressureValue = ((101325 * pow(1 - ((0.0065 * gpsAlt) / (mainTemperatureValue + 273.15)), (9.80665 * 0.0289644) / (8.3144598 * 0.0065))) - (6.112 * exp((17.67 * mainTemperatureValue) / (mainTemperatureValue + 243.5)) * (humidityValue / 100))) / 100;
...
void heatingHandler(bool referenceHeatingEnabledNow, bool humidityModuleHeatingEnabledNow, bool changeModuleHeaterPowerNow) {
  ...
  if (humidityModuleHeating && changeModuleHeaterPowerNow && humidityModuleHeatingEnabledNow && !sensorBoomGeneralError && mainTemperatureValue < 10 && humidityValue > 60) {
  ...
  else if(humidityModuleHeating && humidityModuleHeatingEnabledNow && !changeModuleHeaterPowerNow && mainTemperatureValue < 10 && humidityValue > 60) {
    humidityModuleHeaterPowerControl(heatingPwmCurrentValue);
  }
  else {
    humidityModuleHeaterPowerControl(0);
...
void humidityModuleHeaterPowerControl(unsigned int heaterPower) {  //0 - OFF, 1-255 - only low power heater PWM, 256-500 - low power heater at max and high power heater PWM-controlled
  heatingPwmStatus = map(heaterPower, 0, 500, 0, 200);
...
void setup() {
  ...
  if(humidityModuleHeating) {
    humidityRangeDelta -= 35; //slightly smaller rangeDelta when the heating option is enabled - because the humidity reports 5% lower than without
  }
