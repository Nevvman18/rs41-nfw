...
unsigned int humidityRangeDelta = 1000;  //empirical tests average
...
int defrostingHumidityThreshold = 75; //High humidity environment to activate the defrosting
...
void sensorBoomHandler() {
  ...
  if (sensorBoomEnable) {
    ...
    if (humidityFrequency > zeroHumidityFrequency + 1000 && humidityFrequency < maxHumidityFrequency - 1000) {
      sensorBoomHumidityModuleError = true;
    } else if (humidityValue > 100.0) {
      humidityValue = 100.0;
...
void zeroHumidityFrequencyCalibration() {
  ...
  while(measurementCount < 8) {
    ...
    if(extHeaterTemperatureValue > humidityCalibrationMeasurementTemperature && extHeaterTemperatureValue < humidityCalibrationHeatingTemperature + 5 && !sensorBoomHumidityModuleError) {
      orangeLed();
      delay(60);
      bothLedOff();
...
void heatingHandler(bool referenceHeatingEnabledNow, bool defrostingEnabledNow, unsigned int defrostingOperationTime) {
  ...
  {
    humiditySensorHeaterControl(2); // Start defrosting with high power
