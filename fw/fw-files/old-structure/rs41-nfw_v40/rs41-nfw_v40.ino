  ...
  if (improvedGpsPerformance && gpsSats < 4 && gpsOperationMode != 0 && !cancelGpsImprovement) {
      ...
      for (int i = 0; i < 20; i++) {
        ...
        if (waitTime != 0 && waitTime < 5000) {
          ...
          while (millis() < modeChangeDelayCallbackTimer) {
            if(millis() + 10000 < modeChangeDelayCallbackTimer) {
              modeChangeDelayCallbackTasks();
            }

            if(waitTime > 15000 && millis() + 12000 < modeChangeDelayCallbackTimer) { //if the waiting time is longer than 15 seconds and there is more than 7.5 seconds left
...
void modeChangeDelayCallbackTasks() {
  buttonHandler();
  deviceStatusHandler();
  powerHandler();
  gpsHandler();
...
void heatingHandler(bool referenceHeatingEnabledNow, bool defrostingEnabledNow, unsigned int defrostingOperationTime) {
  if(referenceHeating && referenceHeatingEnabledNow) {
    ...
    else if (cutOutTemp <= 15 && cutOutTemp > 0) {
      selectReferencesHeater(2); //medium power
    }
    else if (cutOutTemp <= 0) {
