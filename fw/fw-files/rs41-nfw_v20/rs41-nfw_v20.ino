...
void buttonHandler() {
  if (analogRead(VBTN_PIN) + 100 > analogRead(VBAT_PIN)) { //new button logic, before VBTN_PIN > 1585 which under lower voltage doesnt work, now reads according to voltage
    if(xdataDebug) {
      xdataSerial.println("Button pressed");
    }
    btnCounter = 0;
    while (btnCounter < 7 && analogRead(VBTN_PIN) + 100 > analogRead(VBAT_PIN)) {
...
float readBatteryVoltage() {
  float batV = ((float)analogRead(VBAT_PIN) / 4095) * 3 * 2 * batVFactor;
  return batV;
}
...
float readThermistorTemp() {
  ...
  float voltage = adcValue * (3.0 / 4095);  // 3.0V reference, 12-bit ADC
