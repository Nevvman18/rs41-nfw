...
bool sensorBoomEnable = true; //enables sensor boom measurement (currently only temperatures, wxHumidity is being engineered) and diagnostics
...
int maxMainTemperature;
int minMainTemperature;
int maxInternalTemp;
int minInternalTemp;
...
void flightComputing() {
  ...
  if(vVCalc > maxAscendRate) {
    maxAscendRate = vVCalc;
  }
  if(vVCalc < maxDescentRate) {
  ...
  if(gpsAlt > flightDetectionAltitude) {
    beganFlying = true;
  }

  if(gpsAlt + burstDetectionThreshold < maxAlt) {
    burstDetected = true;
  }
...
void lowAltitudeFastTxMode() {
  while(millis() - lowAltitudeFastTxModeBeginTime < lowAltitudeFastTxDuration) {
    gpsHandler();
