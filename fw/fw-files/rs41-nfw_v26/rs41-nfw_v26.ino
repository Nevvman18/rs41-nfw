...
bool ubloxM10VerifyDynMod() {
  // Function to read response
  auto readResponse = [&]() -> int {
    uint8_t response[32];           // Buffer for storing response
    int responseIndex = 0;
    uint32_t startTime = millis();  // Start time for timeout handling
    const uint16_t timeout = 1000;  // 1 second timeout

    // Debug: Waiting for response
    if (xdataPortMode == 1) {
      xdataSerial.println("[info] Waiting for response...");
    }

    // Wait for response from the GPS module
    while ((millis() - startTime) < timeout) {
      if (gpsSerial.available()) {
        response[responseIndex++] = gpsSerial.read();

        // Debug: Print each byte as it arrives
        if (xdataPortMode == 1) {
          xdataSerial.print("Received byte: ");
          xdataSerial.println(response[responseIndex - 1], HEX);
        }
    ...
    // If we didn't receive a full UBX response, return 0
    if (responseIndex < 10) {
      if (xdataPortMode == 1) {
        xdataSerial.println("[WARN] Incomplete or no response received.");
      }
      return 0;
    }

    // Debug: Print the entire received response
    if (xdataPortMode == 1) {
      xdataSerial.print("[info] Received response: ");
      for (int i = 0; i < responseIndex; i++) {
        xdataSerial.print(response[i], HEX);
        xdataSerial.print(" ");
      }
      xdataSerial.println();
    }

    // Return dynamic model value (should be in byte 12)
    return response[12];
  };

  // Step 1: Read current dynamic model value
  if (xdataPortMode == 1) {
    xdataSerial.println("[info] Sending UBX-CFG-VALGET command to read current dynamic model...");
  }
  sendUbloxM10(sizeof(ubx_cfg_valget_dyn6), ubx_cfg_valget_dyn6);
  int initialModelValue = readResponse();

  if (initialModelValue == 0) {
    if (xdataPortMode == 1) {
      xdataSerial.println("[WARN] Failed to read initial dynamic model value.");
    }
    return false;
  }

  // Step 2: Send UBX-CFG-VALSET command to set dynamic model to 6
  if (xdataPortMode == 1) {
  ...
  // Step 3: Read the dynamic model value again after setting it
  if (xdataPortMode == 1) {
    xdataSerial.println("[info] Sending UBX-CFG-VALGET command to verify the new dynamic model...");
  }
  sendUbloxM10(sizeof(ubx_cfg_valget_dyn6), ubx_cfg_valget_dyn6);
  int newModelValue = readResponse();

  if (newModelValue == 0) {
    if (xdataPortMode == 1) {
      xdataSerial.println("[WARN] Failed to read new dynamic model value.");
    }
    return false;
  }

  // Step 4: Compare the old and new values
  if (initialModelValue == newModelValue) {
    if (xdataPortMode == 1) {
      xdataSerial.println("[info] Dynamic model has not changed. No update needed.");
    }
    return false;  // No change
  } else {
    if (xdataPortMode == 1) {
      xdataSerial.println("[info] Dynamic model changed successfully.");
    }
