...
* **0 - GNSS fully off.** The receiver is shut down completely and never started. For a stationary station: set the fixed position in `gpsLat` / `gpsLong` / `gpsAlt` and the firmware reports those constant coordinates. With the GNSS disabled the radio-quiet acquisition helper is skipped (there is nothing to acquire), so the radio is never silenced on its account and the status LED shows green when no faults are present, instead of the orange "searching for a fix" state.
...
* **Estimated power draw** (Firmware Builder, RSM4x4): the Power Management section shows a rough current draw at 3 V across the factory battery terminals for the current settings - the ~400-600 mA humidity-check / zero-RH calibration spike in the first minute after power-on, and the average in-flight draw built up from the base PCB (~48 mA) plus each enabled subsystem (RPM411, sensor boom, humidity and reference heating, GPS mode). The radio's ~75 mA transmit draw is listed separately, as it is only pulled during each packet. These are guide figures, not a spec.
